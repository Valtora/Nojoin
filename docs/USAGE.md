...
The recording workspace lays these out in columns as space allows, so that on a wide display everything is visible at once rather than down a long scroll: the capture controls and documents on the left, the live transcript in the middle, and Meeting Edge with your notes on the right. It narrows to two columns and then to a single stack on smaller screens. When you press **Stop**, the transcript column becomes pipeline progress and the rest of the layout stays where it is.
