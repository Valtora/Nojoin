...
- The `mcp:write` scope gates recoverable mutations only: organising recordings (rename, tag, archive, soft delete, restore), task management, transcript corrections (attributed in the edit log with source `mcp` and reversible via further edits), notes regeneration through Nojoin's own pipeline, text-document attachment, calendar linking, and the additive People tools. Nothing under `mcp:write` permanently destroys data or modifies voiceprints, and assistants never author AI-note content directly.
- Permanent recording deletion is gated twice. The separate `mcp:destroy` scope is excluded from the default grant and issued only when the signed-in user ticks the explicit deletion opt-in on the consent page. A client requesting the scope by name has it stripped unless the tick is present: clients re-request previously granted scopes on reconnect, and a stored preference is not consent. Independently, the permanent-delete endpoint refuses any recording not already in the bin, so destruction always requires two deliberate calls with different verbs, on every surface. Grants that predate a scope keep working for everything else; the gated tools refuse them with an instruction to reconnect.
