...
- The `mcp:write` scope gates only additive tools: `import_people` (create/update People records), `set_speaker_name` (name a recording's speaker and link it to a person), and `append_meeting_notes` (append to a recording's user notes). None delete data, modify voiceprints, or alter recordings, transcripts, or AI-generated notes. Grants that predate `mcp:write` keep working for every read tool; the write tools refuse them until the user reconnects and consents to the wider scope.
