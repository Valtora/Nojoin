...
- The `mcp:write` scope gates recoverable mutations only: organising recordings (rename, tag, archive, soft delete, restore), task management (archive at the strongest; task deletion is web-app only), transcript corrections (attributed in the edit log with source `mcp` and reversible via further edits), notes regeneration through Nojoin's own pipeline, text-document attachment, calendar linking, and the additive People tools. Nothing under `mcp:write` permanently destroys data or modifies voiceprints, and assistants never author AI-note content directly.
