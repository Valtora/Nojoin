...
- Permanent recording deletion is gated twice. The separate `mcp:destroy` scope is excluded from the default grant and issued only when the signed-in user ticks the explicit deletion opt-in on the consent page (or a client requests the scope by name, which the consent page then displays). Independently, the permanent-delete endpoint refuses any recording not already in the bin, so destruction always requires two deliberate calls with different verbs, on every surface. Grants that predate a scope keep working for everything else; the gated tools refuse them with an instruction to reconnect.
