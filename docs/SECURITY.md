...
- Permanent recording deletion is gated by the separate `mcp:destroy` scope, which is excluded from the default grant. It is issued only when the signed-in user ticks the explicit deletion opt-in on the consent page (or a client requests the scope by name, which the consent page then displays). Grants that predate a scope keep working for everything else; the gated tools refuse them with an instruction to reconnect.
