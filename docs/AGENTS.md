...
- **Heavy Processing**:
  ...
  - **Pipeline**: Validation -> VAD (Silero) -> Proxy Creation (Alignment) -> Transcribe (Whisper) -> Diarize (Pyannote) -> Merge -> Speaker Resolution (Manual/Merge Check -> LLM) -> Voiceprint Extraction (with outlier filtering) -> Title Inference -> Notes Generation.
