...
- **Heavy Processing**:
  ...
  - **Secondary LLM Provider Fallback**: A secondary LLM provider can be configured via `SECONDARY_LLM_PROVIDER` and related environment variables. When the primary provider fails with any error, `SecondaryLLMBackend` transparently retries the call with the secondary provider. The secondary provider has fully independent configuration (provider, model, live model, API key, Ollama URL). Fallback applies to all AI features: Meeting Edge, meeting intelligence, speaker inference, and chat.
  - **Meeting Edge**: Live guidance is a separate worker path from end-of-processing meeting intelligence. It consumes recent live transcript context plus optional user focus text and user notes, expects a strict JSON contract, and may use a provider-specific Meeting Edge live model that falls back to the provider's main model when unset. Each response also returns a hidden `rolling_summary` (150-300 word running context) that is fed back into the next refresh, and the previous questions/points are passed back so the model retains still-relevant items instead of regenerating near-duplicates. Provider-native JSON modes are used for the Meeting Edge call path only; the tolerant JSON parser remains the fallback contract.
...
- **Lifecycle**:
  ...
  - Navigating between pages within the Nojoin app must not pause capture. A floating recording badge remains visible at the top of the viewport on every page except the recording detail page.
