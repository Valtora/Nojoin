...
- **Heavy Processing**:
  ...
  - **Pipeline**: Validation -> VAD (Silero) -> Proxy Creation (Alignment) -> Transcribe (Whisper) -> Diarize (Pyannote) -> Phantom Speaker Filter -> Merge -> Speaker Resolution (Manual/Merge Check -> LLM) -> Voiceprint Extraction (with outlier filtering) -> Title Inference -> Notes Generation.
