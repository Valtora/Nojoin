...
- **Dashboard Iteration Two:** The root route now serves as an operational dashboard with `Meet Now` capture controls, recent-meeting access, an interactive calendar shell, an inline personal Task List, and clear routing into the tasks, people, and recordings workspaces.
  ...
  - **People Workspace:** The `/people` route provides a dedicated management surface for the shared speaker library, including people records, voiceprints, search, filtering, tagging, and batch maintenance flows.
...
- **Meeting Intelligence:** Provider-gated automatic AI enhancement via one LLM call that can name unresolved speakers (applied automatically without user review), set a meeting title, and write Markdown meeting notes, plus separate Chat Q&A.
...
- **Dashboard Workspace:** The dashboard now combines `Meet Now`, an interactive calendar shell with recorded meeting history, and personal task capture into a single operational home surface.
  - **Calendar Modes:** Month navigation, a `Today` reset action, and an Agenda toggle are live. Without a connected calendar source or unlinked recordings, both modes remain empty-state views.
  ...
  - **Tasks Page:** The main navigation exposes **Tasks** under **Dashboard** and above **People**. This page supports richer task cards with title, body, shared tags, linked recordings, deadlines, completion state, reversible archive, and permanent delete.
