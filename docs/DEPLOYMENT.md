...
- `MCP_ANONYMOUS_DISCOVERY`: Serves the MCP protocol handshake and tool listing on `/mcp` without authentication, so clients that cannot start OAuth from a `401` (Codex Desktop) can connect ([MCP.md](MCP.md)). Exposes only generic tool metadata; no tool executes without an authenticated user. Defaults to `true`; set to `false` to require authentication for every request. Requires an API container restart to change.
- `NOJOIN_TELEMETRY_ENABLED`: Hard switch for anonymous usage data ([TELEMETRY.md](TELEMETRY.md)). Leave unset to manage it from **Settings > Privacy**. Either explicit value overrides the in-app setting and makes the Settings toggle read-only: `false` disables telemetry permanently, and `true` is an explicit opt-in that also stands in for the one-time admin notice, which is suppressed while the environment pins the setting. Set it before first start if telemetry must never be sent from this deployment.
