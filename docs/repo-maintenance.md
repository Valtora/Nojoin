...
- [x] **GOV-002:** Define merge requirements, including required reviewers and required checks for sensitive areas. Required checks by scope and the sensitive-area review obligations are documented in `CONTRIBUTING.md` and `docs/DEVELOPMENT.md`. Branch-protection and required-reviewer enforcement (designed for a sole maintainer, with the seven pull-request CI checks required) are documented as an exact `gh` command and remain a **maintainer-action-pending** GitHub setting. The release jobs are intentionally excluded from required checks because they never run on pull requests; the release pipeline self-gates through its workflow `needs:` graph instead.
