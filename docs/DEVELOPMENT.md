...
- **Duplicate Speaker Merge Pass**: The post-diarisation stage [backend/processing/speaker_merge.py](../backend/processing/speaker_merge.py) is the safety net for over-clustering — pyannote splitting one participant across several speakers. It scores every pair of the recording's active `RecordingSpeaker` voiceprints by cosine similarity and Union-Find-merges any pair at or above `DUPLICATE_SPEAKER_MERGE_THRESHOLD`, with no duration gate (unlike the phantom filter). Note that it scores with a **different** model from the one diarisation clusters with: `pyannote/wespeaker-voxceleb-resnet34-LM` raw cosine here, versus the diarisation pipeline's own embedding and PLDA-based VBx clustering. The two are not calibrated against each other, so the threshold cannot be reasoned about from the diarisation config. Every run emits a `speaker_merge_pass` pipeline metric — including runs that merge nothing, which is the diagnostically important case — carrying each pair's score, the `skip_reason` when the pass could not run, and recording-speaker ids and diarisation labels only (never resolved names, since these lines get pasted into public bug reports). Pair with the `final_diarization_speaker_stats` metric, which records per-speaker speech totals as diarisation left them, to tell genuine over-clustering apart from clusters the phantom filter nearly caught.
