...
**The consent screen says "Connect Codex to Nojoin", and that is correct rather than stale.** The name on that page is whatever the client sends as `client_name` when it registers itself, which Nojoin stores and displays verbatim — it is not a label Nojoin chooses, and no Nojoin release can change it. OpenAI renamed the desktop app to ChatGPT on 9 July 2026 and the client still registers as `Codex`, verified on a freshly built instance with a newly added server (a new server entry means a new registration, so this is not a cached one either). If a future client build registers under a different name, the consent screen will follow it on the next connection with no change here. Existing connections keep the name they registered with; **Settings → Integrations → Connected apps** lists them under it.
