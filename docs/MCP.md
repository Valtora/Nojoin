...
| `get_meeting_analytics` | `mcp:read` | Speaking dynamics for one meeting: talk-time share, turn counts, median and longest turn, directional interruption counts, median response time, a talk-share timeline, and silence and overlap totals, plus measured delivery (pace, pitch and its movement, pausing) where that has been generated. Every figure is measured from the transcript's timings or the audio, never inferred by a model, and matches the app's Analytics tab exactly. Delivery describes how someone spoke and is explicitly not sentiment or mood. Carries `attribution_warnings` when speaker attribution for that recording looks unreliable. |
