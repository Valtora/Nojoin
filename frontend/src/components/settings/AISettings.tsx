...
import { Eye, EyeOff, Check, X, Loader2, Download, Trash2, HelpCircle, Info, RefreshCw, Cpu, Key, Layers, HardDrive, Server } from 'lucide-react';
...
export default function AISettings({ settings, onUpdate, searchQuery = '', isAdmin = false }: AISettingsProps) {
    ...
    // Search Logic
    const showLLM = fuzzyMatch(searchQuery, ['llm', 'provider', 'gemini', 'openai', 'anthropic', 'api key', 'model']);
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 max-w-3xl">
                    ...
                    <div className="space-y-4">
                        <div>
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                        <span className="font-semibold text-gray-900 dark:text-white">
                                            {WHISPER_MODELS.find(m => m.id === (settings.whisper_model_size || 'turbo'))?.label || settings.whisper_model_size}
                                        </span>
                                        <span className="text-sm text-gray-500">
                                            ({WHISPER_MODELS.find(m => m.id === (settings.whisper_model_size || 'turbo'))?.vram} VRAM)
                                        </span>
                                    </div>
                            ...
                            <p className="text-xs text-gray-500 mt-2">
                                Click 'Change Model' to select a different Whisper model variant. You may need to download the new model.
                            </p>
                        </div>
                        <WhisperModelModal
                            ...
                            onUpdate={(newSize) => onUpdate({ ...settings, whisper_model_size: newSize })}
                        />
                    </div>
                </div>
            )}
