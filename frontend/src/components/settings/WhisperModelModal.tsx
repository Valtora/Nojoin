...
import { X, Check, Download, Loader2, AlertTriangle, Cpu, Trash2 } from 'lucide-react';
import { SystemModelStatus } from '@/types';
import { getModelsStatus, downloadModels, getTaskStatus, deleteModel } from '@/lib/api';
...
export default function WhisperModelModal({ isOpen, onClose, currentSize, isAdmin, onUpdate, hfToken }: WhisperModelModalProps) {
    ...
    const handleClearCache = async () => {
        if (!confirm(`Are you sure you want to delete the cached files for the ${selectedModel} model? You will need to download it again to use it.`)) return;
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-gray-700 flex flex-col">
                ...
                <div className="p-6 space-y-6 flex-1">
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 flex gap-3">
                        ...
                        <div>
                            <h4 className="font-medium text-blue-900 dark:text-blue-200">Recommended: Turbo</h4>
                            <p className="text-sm text-blue-800 dark:text-blue-300 mt-1">
                                The <strong>Turbo</strong> model is selected as the default because it offers the best balance between VRAM usage (~6GB), accuracy, and transcription speed.
                                Larger models like 'Large' are only marginally more accurate but significantly slower and require more VRAM (10GB+).
                            </p>
                        </div>
                    </div>
                        ...
                        >
                            {WHISPER_MODELS.map(model => (
                                <option key={model.id} value={model.id}>
                                    {model.label} — {model.params} Params, {model.vram} VRAM, {model.speed} Speed
                                </option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-5 border border-gray-200 dark:border-gray-700">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                {loadingStatus ? (
                                    <span className="text-sm text-gray-500 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Checking...</span>
                                ) : isDownloaded ? (
                        ...
                        {downloading && downloadProgress && (
                            <div className="mb-4 space-y-2">
                                <div className="flex justify-between text-sm">
                                    <span className="text-blue-700 dark:text-blue-300 font-medium">{downloadProgress.message}</span>
                        ...
                        {!isDownloaded && !loadingStatus && (
                            ...
                            >
                                {downloading ? (
                                    ...
                                    <><Download className="w-4 h-4" /> Download {modelInfo?.label} Model</>
                                )}
                            </button>
                        )}
                    </div>
