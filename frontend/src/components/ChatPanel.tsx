...
import { MessageSquare, Send, Trash2, StopCircle, Info, Loader2, Tag as TagIcon, ChevronDown, ChevronUp } from 'lucide-react';
import { useParams } from 'next/navigation';
import { getSettings, getChatHistory, clearChatHistory, streamChatMessage, getUserMe, getTags } from '@/lib/api';
...
export default function ChatPanel() {
    ...
    return (
        <aside id="meeting-chat" className="flex-1 min-w-0 border-l border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 h-full flex flex-col shadow-xl z-10">
                ...
                <div className="flex items-center gap-1">
                    {recordingId && (
                        <button
                            ...
                            className={`flex items-center gap-1 text-xs px-2 py-1.5 rounded-lg border transition-colors ${showTagFilter || selectedTagIds.length > 0
                        ...
                        >
                            ...
                            {showTagFilter ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                        </button>
                    )}
