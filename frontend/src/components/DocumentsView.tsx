...
import { FileText, Upload, Trash2, Download, AlertCircle, Loader2, File } from 'lucide-react';
...
export default function DocumentsView({ recordingId }: DocumentsViewProps) {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'READY': return 'text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400';
            ...
            <div className="flex-1 overflow-y-auto p-6">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-gray-400">
                        ...
                        <p className="text-lg font-medium text-gray-500 dark:text-gray-400">No documents yet</p>
                        <p className="text-sm mt-1 mb-6">Upload PDF, text, or markdown files for context.</p>
                        <button
                            onClick={() => setIsUploadModalOpen(true)}
                        ...
                        {documents.map(doc => (
                            ...
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <div className={`text-xs font-medium px-2 py-0.5 rounded-full ${getStatusColor(doc.status)}`}>
                                        {doc.status}
                                    </div>
                                ...
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <h4 className="font-medium text-gray-900 dark:text-white truncate" title={doc.title}>
                                            {doc.title}
                                        </h4>
