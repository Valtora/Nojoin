...
Section Uninstall
  ...
  ${If} $DeleteAppDataCheckboxState == 1
    ...
    ; Delete by Product Name (Standard Windows behavior)
    RmDir /r "$APPDATA\${PRODUCTNAME}"
    RmDir /r "$LOCALAPPDATA\${PRODUCTNAME}"

    ; Explicitly delete "Nojoin Companion" (Legacy)
    RmDir /r "$LOCALAPPDATA\Nojoin Companion"
    RmDir /r "$APPDATA\Nojoin Companion"
    
    ; Explicitly delete "Nojoin" (New consolidated)
    RmDir /r "$LOCALAPPDATA\Nojoin"
    RmDir /r "$APPDATA\Nojoin"
  ${EndIf}
