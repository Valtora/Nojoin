...
def upgrade() -> None:
    op.add_column(
        "recordings", sa.Column("public_id", sa.String(length=36), nullable=True)
    )
    ...
    op.alter_column(
        "recordings", "public_id", existing_type=sa.String(length=36), nullable=False
    )
    op.create_index(
        op.f("ix_recordings_public_id"), "recordings", ["public_id"], unique=True
    )
