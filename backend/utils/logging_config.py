...
class CheckpointFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        is_speechbrain_debug = (
            record.name == "speechbrain.utils.checkpoints"
...
class SensitiveDataFilter(logging.Filter):
    ...
    _KEY_VALUE_PATTERNS = [
        ...
        re.compile(r"(?i)('x-first-run-password'\s*:\s*')([^']+)(')"),
        re.compile(
            r'(?i)("(?:access_token|anthropic_api_key|api_key|current_password|gemini_api_key|hf_token|new_password|openai_api_key|password|token)"\s*:\s*")([^"]+)(")'
        ),
        re.compile(
            r"(?i)('(?:access_token|anthropic_api_key|api_key|current_password|gemini_api_key|hf_token|new_password|openai_api_key|password|token)'\s*:\s*')([^']+)(')"
        ),
    ...
    _AUTH_SCHEME_PATTERN = re.compile(
        r"(?i)\b(authorization\s*[:=]\s*)(bearer|bootstrap)\s+([^\s,;]+)"
    )
    ...
    def filter(self, record):
        ...
        if record.args:
            if isinstance(record.args, dict):
                ...
                record.args = tuple(
                    self._sanitize_value(None, arg) for arg in record.args
                )
            else:
