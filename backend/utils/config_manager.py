...
DEFAULT_USER_SETTINGS = {
    ...
    # through the user's own subscription (CLI OAuth). Kept out of
    # INSTALL_WIDE_AI_SETTING_KEYS so it is genuinely per-user.
    "usage_model": None,
    # Which connected subscription cli_oauth uses: "claude_code" (default) or
    # "codex". Unset resolves to claude_code so existing cli_oauth users are
    # unaffected.
    "cli_provider": None,
    "cli_model": None,  # Claude CLI model for async tasks (notes/title/speaker/chat)
    "cli_live_model": None,  # Optional lower-latency Claude CLI Meeting Edge model
    "codex_model": None,  # Codex CLI model for async tasks
    "codex_live_model": None,  # Optional lower-latency Codex CLI Meeting Edge model
...
class ConfigManager:
        ...
        try:
            if os.path.exists(self.config_path):
                with open(self.config_path, "r", encoding="utf-8") as f:
                    ...
                    )  # Remove deprecated transcripts directory
                    ...
                    valid_keys = (
                        set(DEFAULT_SYSTEM_CONFIG.keys())
                        | set(DEFAULT_USER_SETTINGS.keys())
                    ) - SENSITIVE_KEYS
                    filtered_config = {
                        k: v for k, v in loaded_config.items() if k in valid_keys
                    }
            ...
            logger.error(
                f"Error decoding JSON from {self.config_path}: {e}. Using default settings.",
                exc_info=True,
            )
            ...
            logger.error(
                f"Error loading configuration: {e}. Using default settings.",
                exc_info=True,
            )
            config = DEFAULT_SYSTEM_CONFIG.copy()  # Reset to defaults on error
            self._ensure_dirs_exist(config)
            ...
            if env_val is not None and env_val != "":
                if config_key.endswith("_context_window"):
                    try:
                        ...
                        logger.warning(
                            "Ignoring invalid integer value for %s.", env_key
                        )
                else:
    ...
    def _ensure_dirs_exist(self, config):
        ...
        if recordings_dir:
            ...
            abs_recordings_dir = path_manager.get_recordings_directory_from_config(
                recordings_dir
            )
            try:
    ...
    def validate_config_value(self, key, value):
        ...
        if key == "whisper_model_size" and value not in WHISPER_MODEL_SIZES:
            raise ValueError(
                f"Invalid whisper_model_size: {value}. Must be one of {WHISPER_MODEL_SIZES}"
            )
        ...
        if key == "meeting_edge_context_level":
            return (
                ...
                and MEETING_EDGE_CONTEXT_LEVEL_MIN
                <= value
        ...
        if key == "llm_provider" and value not in [
            "gemini",
            "openai",
            "anthropic",
            "ollama",
            ...
            raise ValueError(
                f"Invalid llm_provider: {value}. Must be one of ['gemini', 'openai', 'anthropic', 'ollama']"
            )
        if (
            key == "secondary_llm_provider"
            and value is not None
            and value != ""
            and value not in ["gemini", "openai", "anthropic", "ollama"]
        ):
            raise ValueError(
                f"Invalid secondary_llm_provider: {value}. Must be one of ['gemini', 'openai', 'anthropic', 'ollama'] or empty"
            )
...
def get_default_model_for_provider(provider: str) -> str | None:
    ...
    try:
        from backend.processing.llm_services import (
            get_default_model_for_provider as _get_default,
        )
