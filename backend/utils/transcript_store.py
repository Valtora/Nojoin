...
class TranscriptStore:
    ...
    def get(recording_id: str, kind: str = "diarized") -> Optional[str]:
        ...
        if kind not in ["diarized", "raw"]:
            logger.error(
                f"Invalid transcript kind: {kind}. Must be 'diarized' or 'raw'"
            )
            return None
        ...
        try:
            with db_ops.get_db_connection() as conn:
                ...
                cursor.execute(
                    f"SELECT {column} FROM recordings WHERE id = ?", (recording_id,)
                )
                row = cursor.fetchone()
                if row and row[0] is not None:
                    ...
                    logger.warning(
                        f"No {kind} transcript text found for recording {recording_id}"
                    )
                    return None
        except Exception as e:
            logger.error(
                f"Error retrieving {kind} transcript for recording {recording_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    def set(recording_id: str, text: str, kind: str = "diarized") -> bool:
        ...
        try:
            with db_ops.get_db_connection() as conn:
                ...
                column = f"{kind}_transcript_text"
                cursor.execute(
                    f"UPDATE recordings SET {column} = ? WHERE id = ?",
                    (text, recording_id),
                )
                conn.commit()
                if cursor.rowcount == 0:
                    logger.warning(
                        f"No recording found with ID {recording_id} to update {kind} transcript"
                    )
                    return False
                logger.info(
                    f"Successfully stored {kind} transcript for recording {recording_id}"
                )
                return True
        except Exception as e:
            logger.error(
                f"Error storing {kind} transcript for recording {recording_id}: {e}",
                exc_info=True,
            )
            return False
    ...
    def replace(
        recording_id: str,
        replacement_fn: Callable[[str], tuple[str, int]],
        kind: str = "diarized",
    ) -> int:
        """
        ...
        if current_text is None:
            logger.warning(
                f"No {kind} transcript found for recording {recording_id} to perform replacement"
            )
            return -1
        ...
        try:
            ...
            if replacement_count > 0:
                if TranscriptStore.set(recording_id, new_text, kind):
                    logger.info(
                        f"Made {replacement_count} replacements in {kind} transcript for recording {recording_id}"
                    )
                    return replacement_count
                else:
                    logger.error(
                        f"Failed to store updated {kind} transcript for recording {recording_id}"
                    )
                    return -1
            else:
                logger.debug(
                    f"No replacements needed in {kind} transcript for recording {recording_id}"
                )
                return 0
        except Exception as e:
            logger.error(
                f"Error applying replacement function to {kind} transcript for recording {recording_id}: {e}",
                exc_info=True,
            )
            return -1
