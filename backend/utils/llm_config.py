...
# Per-user "usage model" switch (distinct from the install-wide llm_provider).
# Only cli_oauth diverges resolution; other values fall through to llm_provider.
...
def _to_cli_config(
    ...
    return ResolvedLLMConfig(
        ...
        api_key=None,
        model=str(cli_model) if cli_model else None,
        api_url=None,
        context_window=None,
        merged_config=merged,
