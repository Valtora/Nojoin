...
@router.put(
    "/recordings/{recording_id}", response_model=List[RecordingSpeakerPublicRead]
)
async def update_recording_speaker(
    ...
    statement = select(GlobalSpeaker).where(
        GlobalSpeaker.name == update.global_speaker_name,
            ...
            await db.run_sync(
                lambda sync_session: update_recording_speaker_identity(
                    ...
                    target_global_speaker_id=(
                        global_speaker.id if global_speaker is not None else None
                    ),
    ...
    if not recording_speakers:
        raise HTTPException(
            status_code=404,
            detail=f"No speakers found with label {update.diarization_label} in this recording",
        )
    ...
    for rs in recording_speakers:
        if rs.local_name:
            old_names.add(rs.local_name)
        if rs.name:
            old_names.add(rs.name)
                ...
                if global_speaker.embedding:
                    if not global_speaker.is_voiceprint_locked:
                        global_speaker.embedding = merge_embeddings(
                            global_speaker.embedding, rs.embedding, alpha=0.3
                        )
                else:
...
@router.post(
    ...
    response_model=RecordingSpeakerPublicRead,
)
async def promote_speaker_to_global(
    ...
    if not recording_speaker:
        raise HTTPException(
            status_code=404, detail="Speaker not found in this recording"
        )
    ...
    statement = select(GlobalSpeaker).where(
        GlobalSpeaker.name == speaker_name, GlobalSpeaker.user_id == current_user.id
    )
    result = await db.execute(statement)
        ...
        try:
            await db.run_sync(
                lambda sync_session: update_recording_speaker_identity(
                    ...
                    actor_user_id=current_user.id,
                    merge_global_embedding_alpha=(
                        0.5 if existing_global is not None else None
                    ),
                    event_type=(
                        SpeakerCorrectionEventType.LINK_GLOBAL_SPEAKER
...
async def merge_recording_speakers(
    ...
    if not source_speaker:
        raise HTTPException(
            status_code=404,
            detail=f"Source speaker '{merge_data.source_speaker_label}' not found",
        )

    if not target_speaker:
        raise HTTPException(
            status_code=404,
            detail=f"Target speaker '{merge_data.target_speaker_label}' not found",
        )
...
async def update_speaker_color(
    ...
    if not recording_speaker:
        raise HTTPException(
            status_code=404, detail=f"Speaker {label} not found in recording"
        )
...
@router.post(
    ...
    response_model=List[RecordingSpeakerPublicRead],
)
async def split_local_speaker(
    ...
    for rs in all_rec_speakers:
        if (
            rs.local_name == request.new_speaker_name
            or rs.name == request.new_speaker_name
            or rs.diarization_label == request.new_speaker_name
        ):
    ...
    if transcript and transcript_segments:
        ...
        for segment in transcript_segments:
            ...
            for r_id, start, end in segments_to_move:
                if (
                    r_id == recording.public_id
                    and abs(segment["start"] - start) < 0.01
                    and abs(segment["end"] - end) < 0.01
                ):
                    matches = True
                    break
