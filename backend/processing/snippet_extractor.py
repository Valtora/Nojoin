...
def select_clearest_segment(segments: List[Dict], min_length: float = 4.0) -> Dict:
    ...
    long_enough = [
        s for s in segments if (s["end_time"] - s["start_time"]) >= min_length
    ]
    if long_enough:
        # Prefer the one closest to the middle of the recording
        mid = sum([(s["start_time"] + s["end_time"]) / 2 for s in long_enough]) / len(
            long_enough
        )
        long_enough.sort(
            key=lambda s: abs(((s["start_time"] + s["end_time"]) / 2) - mid)
        )
