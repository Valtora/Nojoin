...
from .people_tag import PeopleTagLink
...
class GlobalSpeaker(BaseDBModel, table=True):
    ...
    recording_speakers: List["RecordingSpeaker"] = Relationship(
        back_populates="global_speaker"
    )

    # Tags
    tag_links: List["PeopleTagLink"] = Relationship(
        sa_relationship_kwargs={
            "primaryjoin": "GlobalSpeaker.id==PeopleTagLink.global_speaker_id",
            "lazy": "selectin",
...
class RecordingSpeaker(BaseDBModel, table=True):
    ...
    recording_id: int = Field(
        sa_column=Column(BigInteger, ForeignKey("recordings.id", ondelete="CASCADE"))
    )
    global_speaker_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger, ForeignKey("global_speakers.id", ondelete="SET NULL")
        ),
    )
    ...
    last_speaker_correction_event_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("speaker_correction_events.id", ondelete="SET NULL"),
            index=True,
        ),
    )
    ...
    global_speaker: Optional["GlobalSpeaker"] = Relationship(
        back_populates="recording_speakers"
    )
    ...
    merged_into_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger, ForeignKey("recording_speakers.id", ondelete="SET NULL")
        ),
    )
