...
class RecordingAudioChunk(BaseDBModel, table=True):
    ...
    source_kind: str = Field(default="browser")
...
class RecordingAudioWindowManifest(BaseDBModel, table=True):
    ...
    diarization_config_hash: Optional[str] = Field(
...
class ProcessingRun(BaseDBModel, table=True):
    ...
    parent_run_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger, ForeignKey("processing_runs.id", ondelete="SET NULL")
        ),
    )
    run_kind: ProcessingRunKind = Field(
        default=ProcessingRunKind.BACKFILL,
        sa_column=Column(
            _value_enum_type(ProcessingRunKind, name="processingrunkind"),
            nullable=False,
        ),
    )
    ...
    status: ProcessingRunStatus = Field(
        ...
        sa_column=Column(
            _value_enum_type(ProcessingRunStatus, name="processingrunstatus"),
            nullable=False,
        ),
    )
...
class RecordingAsrWindowResult(BaseDBModel, table=True):
    ...
    chunk_start_sequence: Optional[int] = Field(
        default=None, sa_column=Column(BigInteger)
    )
    chunk_end_sequence: Optional[int] = Field(
        default=None, sa_column=Column(BigInteger)
    )
    ...
    status: RecordingAsrWindowResultStatus = Field(
        ...
        sa_column=Column(
            _value_enum_type(
                RecordingAsrWindowResultStatus, name="recordingasrwindowresultstatus"
            ),
            nullable=False,
        ),
    )
    ...
    error_summary: Optional[str] = Field(default=None, sa_column=Column(Text))
    error_payload: Optional[dict[str, Any]] = Field(
        default=None, sa_column=Column(JSONB)
    )
...
class TranscriptUtterance(BaseDBModel, table=True):
    ...
    recording_speaker_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("recording_speakers.id", ondelete="SET NULL"),
            index=True,
        ),
    )
    state: TranscriptUtteranceState = Field(
        ...
        sa_column=Column(
            _value_enum_type(TranscriptUtteranceState, name="transcriptutterancestate"),
            nullable=False,
        ),
    )
    ...
    last_utterance_event_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("transcript_utterance_events.id", ondelete="SET NULL"),
            index=True,
        ),
    )
    last_diarization_window_result_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("diarization_window_results.id", ondelete="SET NULL"),
    ...
    overlap_group_id: Optional[str] = Field(
    ...
    confidence_payload: Optional[dict[str, Any]] = Field(
        default=None, sa_column=Column(JSONB)
    )
...
class TranscriptUtteranceEvent(BaseDBModel, table=True):
    ...
    utterance_id: int = Field(
        sa_column=Column(
            BigInteger,
            ForeignKey("transcript_utterances.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        )
    )
...
class RecordingSpeakerAlias(BaseDBModel, table=True):
    ...
    recording_speaker_id: int = Field(
        sa_column=Column(
            BigInteger,
            ForeignKey("recording_speakers.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        )
    )
    alias_type: RecordingSpeakerAliasType = Field(
        ...
        sa_column=Column(
            _value_enum_type(
                RecordingSpeakerAliasType, name="recordingspeakeraliastype"
            ),
            nullable=False,
        ),
    )
...
class SpeakerCorrectionEvent(BaseDBModel, table=True):
    ...
    utterance_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("transcript_utterances.id", ondelete="SET NULL"),
            index=True,
        ),
    )
    source_recording_speaker_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("recording_speakers.id", ondelete="SET NULL"),
    ...
    target_recording_speaker_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("recording_speakers.id", ondelete="SET NULL"),
    ...
    target_global_speaker_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("global_speakers.id", ondelete="SET NULL"),
            index=True,
        ),
    )
    event_type: SpeakerCorrectionEventType = Field(
        ...
        sa_column=Column(
            _value_enum_type(
                SpeakerCorrectionEventType, name="speakercorrectioneventtype"
            ),
            nullable=False,
        ),
    )
    scope: SpeakerCorrectionScope = Field(
        ...
        sa_column=Column(
            _value_enum_type(SpeakerCorrectionScope, name="speakercorrectionscope"),
            nullable=False,
        ),
    )
...
class DiarizationWindowTurn(BaseDBModel, table=True):
    ...
    window_result_id: int = Field(
        sa_column=Column(
            BigInteger,
            ForeignKey("diarization_window_results.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        )
    )
    local_speaker_key: str = Field(
        sa_column=Column(String(255), nullable=False, index=True)
    )
    start_ms: int = Field(sa_column=Column(BigInteger, nullable=False))
    end_ms: int = Field(sa_column=Column(BigInteger, nullable=False))
    confidence: Optional[float] = None
    matched_recording_speaker_id: Optional[int] = Field(
        default=None,
        sa_column=Column(
            BigInteger,
            ForeignKey("recording_speakers.id", ondelete="SET NULL"),
