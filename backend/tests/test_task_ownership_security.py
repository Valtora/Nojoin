...
from backend.models.task import AsyncTaskOwnership, register_task_ownership, check_task_ownership
...
async def test_task_ownership_lifecycle_and_security(test_session_maker) -> None:
    ...
    # Mock users
    user_a = User(id=101, username="user_a", hashed_password="pw", role=UserRole.USER, is_superuser=False)
    user_b = User(id=102, username="user_b", hashed_password="pw", role=UserRole.USER, is_superuser=False)
    admin_user = User(id=103, username="admin_user", hashed_password="pw", role=UserRole.ADMIN, is_superuser=False)
