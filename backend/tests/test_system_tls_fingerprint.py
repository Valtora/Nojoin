...
def test_get_tls_fingerprint_dynamic_fetch_avoids_sensitive_logging(
    monkeypatch, caplog
):
    ...
    monkeypatch.setattr(
        system, "get_trusted_web_origin", lambda: "https://nojoin.example.com"
    )
    monkeypatch.setattr(
        ssl, "create_default_context", lambda: _DummySSLContext(cert_der)
    )
    monkeypatch.setattr(
        socket, "create_connection", lambda *args, **kwargs: _DummySocket()
    )
...
def test_get_tls_fingerprint_local_fallback_avoids_sensitive_logging(
    monkeypatch, caplog
):
    ...
    monkeypatch.setattr(
        ssl, "create_default_context", lambda: _DummySSLContext(b"unused")
    )
    ...
    monkeypatch.setattr(
        os.path, "exists", lambda path: path == "/etc/nginx/certs/cert.crt"
    )
    monkeypatch.setattr(ssl, "PEM_cert_to_DER_cert", lambda pem_data: cert_der)
    monkeypatch.setattr(
        builtins, "open", lambda *args, **kwargs: io.StringIO("pem-data")
    )
