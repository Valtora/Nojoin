...
def create_recording_row(
    engine, *, recording_id: int, audio_path: str, status: str = "PROCESSED"
) -> None:
...
def test_generate_proxy_task_reuses_imported_mp3_without_deleting_it(
    ...
    def fail_if_called(*args, **kwargs):
        raise AssertionError(
            "convert_to_proxy_mp3 should not run for same-path MP3 imports"
        )
    ...
    try:
        with Session(verification_engine) as session:
            recording = session.exec(
                text("SELECT proxy_path, audio_path FROM recordings WHERE id = 1")
            ).one()
...
def test_generate_proxy_task_mixdowns_browser_capture_for_proxy(
    ...
    create_recording_row(
        engine, recording_id=2, audio_path=str(audio_path), status="QUEUED"
    )

    convert_calls: list[tuple[str, str, bool]] = []

    def fake_convert_to_proxy_mp3(
        input_path: str, output_path: str, *, mix_to_mono: bool = False
    ) -> bool:
    ...
    monkeypatch.setattr(
        "backend.utils.audio.convert_to_proxy_mp3", fake_convert_to_proxy_mp3
    )
    monkeypatch.setattr(
        tasks_module,
        "_recording_uses_browser_capture",
        lambda session, recording_id: True,
    )
    monkeypatch.setattr(tasks_module, "get_sync_session", lambda: Session(engine))
    tasks_module.generate_proxy_task._session = None
    ...
    try:
        with Session(verification_engine) as session:
            recording = session.exec(
                text("SELECT proxy_path, audio_path FROM recordings WHERE id = 2")
            ).one()
